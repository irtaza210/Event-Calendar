...
// gets all the events according to the logged in user and stores the event into an array called data. this array has all the specifics of
// the event such as event name, date, time, category. all these specifics are displayed on the page using a table
$stmt = mysqli_query($database, "SELECT * FROM events where username = '$user_id' order by date,time");
$data = array();
